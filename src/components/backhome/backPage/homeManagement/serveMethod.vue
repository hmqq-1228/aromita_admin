<template>
  <div class="banner">
    <div class="header">
      <div>特色服务 (最多创建5条)</div>
      <el-button type="primary" icon="el-icon-plus" v-if="bannerList.length<5" @click="addNew()">新增</el-button>
    </div>
    <el-table
      :data="bannerList"
      border
      ...
      <el-table-column label="操作" width="280">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="danger" v-if="bannerList.length>3" @click="delItem(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-drawer
      ...
      <div class="draCont">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px">
          ...
          <el-form-item label="上传图片" prop="imageUrl">
            ...
            <div class="tip"><i class="el-icon-warning-outline"></i> 请上传不超过500KB的260*60尺寸的PNG、JPG、JPEG格式图片!</div>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods:{
      // 获取banner列表
      getBannerListFuc () {
        getBannerList ({position:50}).then((res)=>{
      ...
      toBannerDetail (id) {
        ...
        this.$axios.get(`backend/home/${id}`,{}).then((res)=>{
          ...
          if (res.data.code === 200) {
            ...
            if (res.data.data.position === 50) {
              this.DetailPosion = '特色服务'
            }
