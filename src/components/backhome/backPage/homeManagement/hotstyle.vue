<template>
  <div class="banner">
    <div class="header">
      <div>Hot Style in Social Medias (最多添加8个)</div>
      <el-button type="primary" icon="el-icon-plus" v-if="bannerList.length<8" @click="addNew()">新增</el-button>
    </div>
    <el-table
      :data="bannerList"
      border
...
<script>
  ...
  export default {
    ...
    methods:{
      // 获取banner列表
      getBannerListFuc () {
        getBannerList ({position:30}).then((res)=>{
      ...
      toBannerDetail (id) {
        ...
        this.$axios.get(`backend/home/${id}`,{}).then((res)=>{
          ...
          if (res.data.code === 200) {
            ...
            if (res.data.data.position === 30) {
              this.DetailPosion = 'Hot Style in Social Medias '
            }
