<template>
  <div class="banner">
    <div class="header">
      <div>首页collections主图(只能添加1个)</div>
...
<script>
  ...
  export default {
    ...
    methods:{
      // 获取banner列表
      getBannerListFuc () {
        getBannerList ({position:21}).then((res)=>{
      ...
      addNew () {
        ...
        this.isEdit = false
        this.bntStr = '立即创建'
        this.bannerTitle = '新建Collections主图'
      },
      addNewFuc () {
        var obj = {
          position: 21,
      ...
      bannerEdit (id) {
        ...
        that.isEdit = true
        that.bntStr = '确定修改'
        that.bannerTitle = '修改Collections主图'
